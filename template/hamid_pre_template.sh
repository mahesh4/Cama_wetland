...
EXP="hamid"                    #   experiment name (output directory name)
...
##### Simulation Time #################
YSTART=<SYEAR>                       #   start year (from YSTART / Jan / 1st )
YEND=<EYEAR>
...
##### Map & Topography ################
FMAP=${BASE}/map/$EXP       #   map directory
# FMAP="${BASE}/map/region_15min"       # (regional map) 

CDIMINFO="${FMAP}/diminfo_0625.txt"   #   dimention info (1deg, 0E->360E, 90N-90S)
...
CPMANRIV=${FMAP}/rivman.bin            #   channel Manning's coefficient
CPMANFLD=${FMAP}/fldman.bin            #   floodplain Manning's coefficient
...
CINPMAT=${FMAP}/inpmat-0625.bin            #   runoff input matrix (1deg, 0E->360E, 90N-90S)     !! for sample bonary input
...
CROFDIR=${BASE}/inp/$EXP/      #   runoff directory
CROFPRE="Roff___"                         #   runoff prefix/suffix  ( $(PREFIX)yyyymmdd$(SUFFIX) )
CROFSUF=".bin"
...
CPMANRIV="${CPMANRIV}"                ! channel Manning's coefficient
CPMANFLD="${CPMANFLD}"                ! floodplain Manning's coefficient
...
echo "saving the output to dropbox"
python ${APIDIR}/dropbox_connect.py

echo "deleting any previous generated output in the server"
rm -rf ${BASE}/out/*
